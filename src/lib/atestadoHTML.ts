...
export function gerarHTMLAtestado(p: AtestadoHTMLParams, comAutoprint = false): string {
  const { paciente, medico, dias, dataInicio, dataFim, dataEmissao, cid, textoComplementar, observacoes } = p
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .doc-info{font-size:8pt;color:#999;text-align:right}
  ...
  .hl{font-weight:600;color:#19382E}
  .cid-box{display:inline-block;background:#F0F9F5;border:1px solid #6E8570;border-radius:6px;padding:4px 12px;font-size:9pt;color:#19382E;font-weight:600;margin-bottom:16px}
...
<div class="page">
  <div class="header">
    ...
    <div class="doc-info">Emitido em: ${fmtData(dataEmissao)}<br/>Documento médico oficial</div>
  </div>
  ...
  <p class="body-text">
    Atesto, para os devidos fins, que ${sexoPac} <span class="hl">${paciente.nome}</span>${paciente.cpf ? `, portador(a) do CPF nº <span class="hl">${paciente.cpf}</span>,` : ','}
    ...
    <span class="hl">${dias} (${diasExt}) ${dias === 1 ? 'dia' : 'dias'}</span>,
    a contar de <span class="hl">${fmtData(dataInicio)}</span> a <span class="hl">${fmtData(dataFim)}</span>.
  </p>
...
export function nomeArquivo(paciente: { nome: string }, dataEmissao: string): string {
  const nome = paciente.nome.replace(/\s+/g, '-').toLowerCase()
  return `atestado-${nome}-${dataEmissao}.html`
}

/** Abre janela de impressão */
export function imprimirAtestado(params: AtestadoHTMLParams): void {
  const html = gerarHTMLAtestado(params, true)
  const win = window.open('', '_blank', 'width=900,height=700')
  if (win) { win.document.write(html); win.document.close() }
}

/** Faz download real do arquivo HTML (sem abrir diálogo de impressão) */
export function baixarAtestado(params: AtestadoHTMLParams): void {
  const html = gerarHTMLAtestado(params, false)
  const blob = new Blob([html], { type: 'text/html;charset=utf-8' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = nomeArquivo(params.paciente, params.dataEmissao)
...
/** Cria um File blob pronto para compartilhamento */
export function criarArquivoAtestado(params: AtestadoHTMLParams): File {
  const html = gerarHTMLAtestado(params, false)
  const blob = new Blob([html], { type: 'text/html' })
  return new File([blob], nomeArquivo(params.paciente, params.dataEmissao), { type: 'text/html' })
}

export function textoResumido(params: AtestadoHTMLParams): string {
  const { paciente, medico, dias, dataInicio, dataFim, cid, textoComplementar } = params
  const di = new Date(dataInicio + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })
  const df = new Date(dataFim + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })
  return [
    `Atestado Médico — Aduno`,
