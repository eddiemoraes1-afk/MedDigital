...
export default function ReceitaShareModal({ params, onClose }: Props) {
  ...
  const assunto = encodeURIComponent(`${tipoLabel} — Aduno`)
  ...
  async function compartilharNativo() {
    ...
    try {
      ...
      if (typeof navigator.canShare === 'function' && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: `${tipoLabel} — Aduno` })
        onClose()
        return
      }
      if ('share' in navigator) {
        await navigator.share({ title: `${tipoLabel} — Aduno`, text: texto })
        onClose()
      }
    } catch (e: any) {
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <p className="font-semibold text-[#19382E] text-sm">Encaminhar receita</p>
