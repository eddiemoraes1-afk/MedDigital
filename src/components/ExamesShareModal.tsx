...
function textoResumido(params: ExamesHTMLParams): string {
  ...
  return [
    ...
    `Data: ${new Date(params.dataSolicitacao + 'T12:00:00').toLocaleDateString('pt-BR')}`,
  ].filter(Boolean).join('\n')
}

export default function ExamesShareModal({ params, onClose }: Props) {
  ...
  const nomeDoc  = nomeArquivoExames(params.paciente, params.dataSolicitacao)
        ...
        <div className="p-4 space-y-2">
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-gray-400">Copia os dados da solicitação</p>
            </div>
          </button>
        </div>
      </div>
