...
import { Clock, Save, Loader2, CheckCircle2, Trash2, Pencil, X } from 'lucide-react'
...
export default function DisponibilidadePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F3F6F3]">
      <MedicoHeader titulo="Disponibilidade" backHref="/medico/dashboard" />
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6">
          <h2 className="font-bold text-[#19382E] mb-4">Adicionar disponibilidade</h2>
          ...
          <button
            ...
            disabled={salvando}
            className="flex items-center gap-2 bg-[#19382E] text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-[#6E8570] disabled:opacity-50"
          >
          ...
          {DIAS.map((dia, idx) => {
            ...
            return (
              <div key={dia} className="bg-white rounded-2xl p-5 shadow-sm">
                <h3 className="font-semibold text-[#19382E] mb-3">{dia}</h3>
                <div className="space-y-2">
                  {horariosDodia.map(h => (
                    <div key={h.id} className={`rounded-xl border transition-all ${h.ativo ? 'border-green-100 bg-green-50' : 'border-gray-100 bg-gray-50 opacity-60'} ${editandoId === h.id ? 'border-[#6E8570] bg-white opacity-100' : ''}`}>
                      {editandoId === h.id ? (
                        /* Modo edição inline */
                        <div className="p-4">
                          <p className="text-xs font-semibold text-[#19382E] mb-3">Editando horário de {dia}</p>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex items-center gap-1.5 bg-[#19382E] text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-[#6E8570]"
                            >
