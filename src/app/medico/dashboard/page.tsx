    ...
    return (
      <div className="min-h-screen bg-[#F4F7FB] flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl p-8 max-w-md text-center shadow-sm">
          ...
          <h2 className="text-xl font-bold text-[#1A3A5C] mb-2">Cadastro em análise</h2>
          ...
          <Link href="/login" className="mt-4 inline-block text-[#2E75B6] text-sm">Sair</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB]">
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <form action="/api/auth/signout" method="POST">
              <button type="submit" className="text-blue-200 hover:text-white">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[#1A3A5C]">Bom dia, Dr(a). {primeiroNome}!</h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Na fila agora', valor: fila?.length || 0, icon: Clock, cor: '#2E75B6', destaque: (fila?.length || 0) > 0 },
            ...
            <div key={item.label} className={`bg-white rounded-2xl p-6 shadow-sm ${item.destaque ? 'ring-2 ring-[#2E75B6]' : ''}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-bold text-[#1A3A5C]">Fila de Atendimento Virtual</h2>
            </div>
            {fila && fila.length > 0 && (
              <span className="bg-[#2E75B6] text-white text-xs font-bold px-2.5 py-1 rounded-full">
              ...
              {fila.map((atendimento: any, index: number) => {
                ...
                return (
                  <div key={atendimento.id} className="px-6 py-5 flex items-center justify-between hover:bg-gray-50">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-[#1A3A5C]/10 rounded-full flex items-center justify-center font-bold text-[#1A3A5C] text-sm">
                        {index + 1}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                    ...
                    <Link
                      ...
                      className="bg-[#1A3A5C] hover:bg-[#2E75B6] text-white px-5 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 shrink-0"
                    >
