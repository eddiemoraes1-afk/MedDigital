...
export default function ContasReceberTab() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [rP, rA] = await Promise.all([
        fetch('/api/admin/financeiro/lancamentos?tipo=receita&status=pendente&limite=500'),
        fetch('/api/admin/financeiro/lancamentos?tipo=receita&status=atrasado&limite=500'),
      ])
  ...
  const gerarMensalidades = async () => {
    ...
    try {
      ...
      alert(d.mensagem ?? `${d.gerados} mensalidade(s) gerada(s)`)
      await carregar()
  ...
  const exportExcel = () => exportarExcel(linhasExcel(), 'Contas_a_Receber', 'Contas a Receber')

  const exportPDF = () => {
    const colunas = ['Status', 'Descrição', 'Empresa', 'Categoria', 'Valor', 'Vencimento']
    ...
    linhas.push({
      cells: [
        '', `Total: ${todos.length} faturas`,
        '', '',
        `Atrasado: ${fmtBRL(totalAtrasado)} | Pendente: ${fmtBRL(totalPendente)} | TOTAL: ${fmtBRL(totalAtrasado + totalPendente)}`,
        '',
      ],
      negrito: true, corFundo: '1A3A2C', corTexto: 'FFFFFF',
    })
    exportarPDF('Contas a Receber', colunas, linhas)
  }
  ...
  const Linha = ({ l, cor }: { l: Lancamento; cor: string }) => (
    <tr style={{ borderBottom: '1px solid var(--border)' }}>
      <td className="px-4 py-3">
        ...
        <p className="text-xs" style={{ color: 'var(--txt-muted)' }}>{l.empresas?.nome ?? ''}{l.categorias_financeiras ? ` · ${l.categorias_financeiras.nome}` : ''}</p>
      </td>
      ...
      <td className="px-4 py-3">
        <button onClick={() => marcarRecebido(l.id)}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-5">
        <div className="rounded-2xl p-4 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-2xl font-bold text-yellow-600">{fmtBRL(totalPendente)}</p>
          <p className="text-xs mt-0.5" style={{ color: 'var(--txt-muted)' }}>{pendFiltrados.length} fatura{pendFiltrados.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="rounded-2xl p-4 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--txt-muted)' }}>{atrFiltrados.length} fatura{atrFiltrados.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-2 items-center">
        <div className="relative">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar descrição / empresa..."
      ...
      <div className="rounded-2xl border p-3 mb-5 flex flex-wrap items-center gap-3" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
        ...
        <span className="text-xs" style={{ color: 'var(--txt-muted)' }}>Ignora duplicatas</span>
      </div>
            ...
            {pendFiltrados.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>{pendFiltrados.map(l => <Linha key={l.id} l={l} cor="text-yellow-600" />)}</tbody>
                </table>
              </div>
            )}
          </div>
