...
export default function ContasPagarTab() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [rPend, rAtr] = await Promise.all([
        fetch('/api/admin/financeiro/lancamentos?tipo=despesa&status=pendente&limite=200'),
        fetch('/api/admin/financeiro/lancamentos?tipo=despesa&status=atrasado&limite=200'),
      ])
  ...
  const gerarFolha = async () => {
    ...
    try {
      ...
      const msg = d.mensagem ?? `${d.gerados} médico(s) — Total: ${d.total_honorarios ? fmt(d.total_honorarios) : 'R$ 0,00'}`
      alert(msg)
      await carregar()
    } finally {
      setGerando(false)
  ...
  const Linha = ({ l, cor }: { l: Lancamento; cor: string }) => (
    <tr style={{ borderBottom: '1px solid var(--border)' }}>
      <td className="px-4 py-3">
        ...
        <p className="text-xs" style={{ color: 'var(--txt-muted)' }}>{l.medicos?.nome ?? ''} · {l.categorias_financeiras?.nome ?? 'Sem cat.'}</p>
      </td>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="rounded-2xl p-4 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--txt-muted)' }}>{pendentes.length} conta{pendentes.length !== 1 ? 's' : ''}</p>
        </div>
        <div className="rounded-2xl p-4 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--txt-muted)' }}>{atrasados.length} conta{atrasados.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border p-4 mb-6 flex flex-wrap items-center gap-3" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
        ...
        <span className="text-sm font-medium" style={{ color: 'var(--txt)' }}>Gerar folha de honorários médicos:</span>
        ...
        <span className="text-xs" style={{ color: 'var(--txt-muted)' }}>
          Baseado nas consultas concluídas no período (ignora duplicatas)
        </span>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
            ...
            {pendentes.length === 0 ? (
              <p className="px-4 py-8 text-center text-sm" style={{ color: 'var(--txt-muted)' }}>Nenhuma conta pendente a pagar</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead><tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--card)' }}>
                  ...
                  <tbody>{pendentes.map(l => <Linha key={l.id} l={l} cor="text-orange-600" />)}</tbody>
                </table>
              </div>
            )}
          </div>
